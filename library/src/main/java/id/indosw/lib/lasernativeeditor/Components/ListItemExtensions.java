...
import static id.indosw.lib.lasernativeeditor.Components.InputExtensions.CONTENT;
...
public class ListItemExtensions extends EditorComponent {
                    ...
                    if (s.length() > 0) {
                        if (s.charAt(s.length() - 1) == '\n') {
                            ...
                            if (s.length() == 0 || s.toString().equals("\n")) {
                                ...
                                @SuppressWarnings("unused") int index = _table.indexOfChild(_row);
    ...
    public String getTextFromListItem(View row) {
        ...
        return Objects.requireNonNull(_text.getText()).toString();
    }
        ...
        if (currentFocus == EditorType.UL_LI && !isOrdered) {
            ...
            if (view != null) {
                ...
                if (type == EditorType.INPUT) {
                    String text = Objects.requireNonNull(((CustomEditText) view).getText()).toString();  //get the text, if not null, replace it with list item
